...
@section('styles')
    <style>
        ...
        .hero-badge.right.active {
            background: var(--green-soft);
            color: var(--green);
            border: 1px solid rgba(16, 185, 129, 0.2);
        }

        .hero-badge.right.inactive {
            background: var(--red-soft);
            color: var(--red);
            border: 1px solid rgba(239, 68, 68, 0.2);
        }

        .hero-badge.right.utama {
        ...
        .modal-box {
            ...
            max-width: 400px;
            width: 100%;
            box-shadow: 0 24px 64px rgba(15, 30, 54, 0.25);
...
@section('content')
            ...
            <div class="vehicle-hero" id="vehicleHero">
                ...
                @else
                    <div class="hero-badge right active" id="heroBadgeStatus">
                        <span
                            style="width:7px;height:7px;background:var(--green);border-radius:50%;display:inline-block"></span>
                        Aktif
                    </div>
                @endif
                ...
                <div class="hero-car-wrap" id="heroCarWrap">
                    ...
                    <svg id="carSvgMobil" width="280" height="140" viewBox="0 0 300 150" fill="none"
                        ...
                        <path d="M176 45 L176 30 L192 30 Q214 30 225 45Z" fill="#bfdbfe" stroke="#93c5fd"
                        ...
                        <line x1="172" y1="28" x2="174" y2="102" stroke="#c8d4e8"
                            stroke-width="1.5" />
                        <circle cx="82" cy="103" r="22" fill="#475569" stroke="#334155"
                            stroke-width="2" />
                    ...
                    {{-- Motor SVG --}}
                    <svg id="carSvgMotor" width="240" height="140" viewBox="0 0 280 150" fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="hero-vehicle-sub">{{ ucfirst($kendaraan->jenis) }} &bull;
                    {{ $kendaraan->warna ?? 'Warna tidak diatur' }}</div>
    ...
    ════════════════════════════════ --}}
            <form action="{{ route('user.kendaraan.update', $kendaraan->id) }}" method="POST" id="formKendaraan">
                ...
                <input type="hidden" name="warna" id="inputWarna"
                    value="{{ old('warna', $kendaraan->warna ?? 'Putih') }}">
                <input type="hidden" name="utama" id="inputUtama"
                    value="{{ old('utama', $kendaraan->utama ? '1' : '0') }}">
                    ...
                    <select class="form-select {{ $errors->has('merek') ? 'is-invalid' : '' }}" id="merekKendaraan"
                        ...
                        <optgroup label="Mobil" id="groupMobil">
                            @foreach (['BMW', 'Toyota', 'Honda', 'Suzuki', 'Mitsubishi', 'Daihatsu', 'Mercedes-Benz', 'Audi', 'Hyundai', 'Kia', 'Wuling', 'MG'] as $b)
                                <option value="{{ $b }}"
                                    {{ old('merek', $kendaraan->merek) === $b ? 'selected' : '' }}>{{ $b }}
                                </option>
                            @endforeach
                        </optgroup>
                        ...
                        <option value="Lainnya" {{ old('merek', $kendaraan->merek) === 'Lainnya' ? 'selected' : '' }}>
                            Lainnya</option>
                    </select>
                    @error('merek')
                ...
                <div class="form-group">
                    ...
                    <input class="form-field {{ $errors->has('model') ? 'is-invalid' : '' }}" type="text"
                        id="modelKendaraan" name="model" placeholder="Contoh: Avanza G, NMAX 155..."
                        value="{{ old('model', $kendaraan->model) }}" list="modelSuggestions" autocomplete="off"
                ...
                <div class="form-group">
                    <div class="form-label">Warna Kendaraan <span
                            style="font-size:10px;color:var(--text-muted);font-weight:500;background:var(--bg-input);padding:1px 7px;border-radius:999px;">Opsional</span>
                    </div>
                    <div class="color-row" id="colorPicker"></div>
                    <div class="color-hint">Warna dipilih: <strong
                            id="colorName">{{ $kendaraan->warna ?? 'Putih' }}</strong></div>
                </div>
                ...
                <div class="checkbox-card {{ old('utama', $kendaraan->utama) ? 'checked' : '' }}" id="checkboxCard"
    ...
    {{-- Hidden form DELETE --}}
    <form id="formDelete" action="{{ route('user.kendaraan.destroy', $kendaraan->id) }}" method="POST"
        style="display:none">
        @csrf
        @method('DELETE')
    </form>
...
@section('scripts')
    ...
    ═══════════════════════════════════════ */
        ...
        function updateHeroName() {
            ...
            document.getElementById('heroName').textContent = merek && model ? merek + ' ' + model : merek ||
                'Nama Kendaraan';
        }
