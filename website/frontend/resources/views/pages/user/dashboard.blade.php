...
@section('styles')
    ...
    <style>
        ...
        .all-loc-card:nth-child(1) {
            animation-delay: 0ms;
        }

        .loc-card:nth-child(2),
        .all-loc-card:nth-child(2) {
            animation-delay: 50ms;
        }

        .loc-card:nth-child(3),
        .all-loc-card:nth-child(3) {
            animation-delay: 100ms;
        }

        .loc-card:nth-child(4),
        .all-loc-card:nth-child(4) {
            animation-delay: 150ms;
        }

        .loc-card:nth-child(5),
        .all-loc-card:nth-child(5) {
            animation-delay: 200ms;
        }

        .loc-card:nth-child(6),
        .all-loc-card:nth-child(6) {
...
@section('scripts')
    <script>
        ...
        function getFilteredLocs() {
            let locs = parkingLocations.map(l => ({
                ...l
            }));
            if (searchQuery) {
                locs = locs.filter(l =>
                    l.name.toLowerCase().includes(searchQuery) ||
                    l.addr.toLowerCase().includes(searchQuery)
                );
            }
        ...
        const rekoConfig = {
            'Terdekat': {
                ...
                label: 'Paling Dekat dari Lokasimu',
                sub: loc => jarakStr(loc.jarak) + ' dari kamu'
            },
            'Tersedia': {
                icon: 'fa-circle-check',
                label: 'Slot Paling Banyak Tersedia',
                sub: loc => loc.slots + ' slot tersedia'
            },
            'Termurah': {
                icon: 'fa-tag',
        ...
        function renderCards() {
                ...
                <div class="reko-banner">
                    ...
                    if (map && parkingLocations.length) {
                        const bounds = parkingLocations
                            ...
                            .map(l => [l.lat, l.lng]);
                        if (bounds.length) map.flyToBounds(bounds, {
                            padding: [40, 40],
                            animate: true,
                            duration: 1
                        });
            ...
            if (locScroll) {
                if (!filtered.length) {
                    locScroll.innerHTML = `<div style="color:var(--text-muted);font-size:13px;padding:20px 0">
    ${searchQuery ? `Tidak ada hasil untuk "<b>${searchQuery}</b>"` : 'Tidak ada lokasi sesuai filter.'}
</div>`;
                } else {
                    locScroll.innerHTML = '';
            ...
            if (allLocGrid) {
                if (!filtered.length) {
                    allLocGrid.innerHTML = `<div style="color:var(--text-muted);font-size:13px;padding:20px 0;grid-column:1/-1">
    ${searchQuery ? `Tidak ada hasil untuk "<b>${searchQuery}</b>"` : 'Tidak ada lokasi sesuai filter.'}
</div>`;
                } else {
                    allLocGrid.innerHTML = '';
                    filtered.forEach((loc, i) => {
                        const statusCls = loc.status === 'avail' ? 'avail-text' : loc.status === 'busy' ?
                            'busy-text' : 'full-text';
