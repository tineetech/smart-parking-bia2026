...
<title>Parkify — Cari Lokasi</title>
...
.desktop-nav {
  ...
  background: rgba(255,255,255,0.9); backdrop-filter: blur(12px);
  border: 1px solid rgba(255,255,255,0.7);
  border-radius: 14px; padding: 4px; box-shadow: var(--shadow-sm);
}
.desktop-nav a {
...
.desktop-nav a.active { background: var(--blue-main); color: #fff; font-weight: 600; }
...
.leaflet-popup-content-wrapper {
  ...
  padding: 0 !important; overflow: hidden; border: 1px solid var(--border);
}
.leaflet-popup-content { margin: 0 !important; width: auto !important; }
.custom-popup { padding: 12px 14px; min-width: 190px; }
.popup-name {
  font-family: 'Space Grotesk', sans-serif;
  font-size: 13px; font-weight: 700; color: var(--text-primary); margin-bottom: 3px;
}
.popup-addr { font-size: 11px; color: var(--text-muted); margin-bottom: 8px; }
.popup-meta { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; margin-bottom: 8px; }
.popup-badge {
  font-size: 11px; font-weight: 600;
  padding: 2px 9px; border-radius: 999px; font-family: 'Poppins', sans-serif;
}
...
.popup-price { font-size: 12px; color: var(--blue-main); font-weight: 700; }
.popup-btn {
  display: block; width: 100%; background: var(--blue-main); color: #fff;
...
.content-panel {
  ...
  max-width: var(--max-content); margin: -32px auto 0;
  background: #ffffff;
...
.search-loc-btn {
  ...
  background: var(--blue-soft); border: none;
  display: flex; align-items: center; justify-content: center;
  cursor: pointer; color: var(--blue-main); flex-shrink: 0;
...
.pill:hover, .pill.active {
  background: var(--blue-main); border-color: var(--blue-main); color: #fff;
}
...
/* ═══════════════════════════════════════
   LOCATION CARDS
═══════════════════════════════════════ */
.loc-list { padding: 0 20px; display: flex; flex-direction: column; gap: 14px; }
...
.loc-card:hover {
  ...
  box-shadow: var(--shadow-md); border-color: var(--blue-pale);
}
...
.loc-card:hover .loc-img-wrap img { transform: scale(1.03); }

.loc-img-placeholder {
  width: 100%; height: 100%;
  display: flex; align-items: center; justify-content: center;
  font-size: 52px; background: #f8fafc;
}
...
.img-badge {
  ...
  font-family: 'Poppins', sans-serif; backdrop-filter: blur(6px);
  display: flex; align-items: center; gap: 4px;
}
.img-badge.avail { background: rgba(236,253,245,0.92); color: var(--green); }
.img-badge.busy  { background: rgba(255,251,235,0.92); color: var(--amber); }
.img-badge.full  { background: rgba(254,242,242,0.92); color: var(--red); }
...
.loc-body { padding: 14px 16px 16px; }
.loc-name {
  font-family: 'Space Grotesk', sans-serif;
...
.loc-tags { display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: 12px; }
.loc-tag {
...
.loc-price-wrap { display: flex; align-items: center; gap: 6px; }

.price-tag {
  display: flex; align-items: center; gap: 5px;
  background: var(--blue-soft); border: 1px solid var(--blue-pale);
  color: var(--blue-main); border-radius: 9px; padding: 5px 10px;
...
.slot-tag.has-slot { background: var(--green-soft); border-color: #a7f3d0; color: var(--green); }
.slot-tag.few-slot { background: var(--amber-soft); border-color: #fde68a; color: var(--amber); }
.slot-tag.no-slot  { background: var(--red-soft);   border-color: #fecaca; color: var(--red); }
...
@media (min-width: 860px) {
  ...
  #parkingMap { height: 480px; }
  .loc-list { display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; }
  .loc-img-wrap { height: 180px; }
  .desktop-nav { display: flex; }
  .hamburger-btn { display: none; }
  .user-name-text { display: block !important; }
  .map-wrapper::after { display: none; }
  .content-panel { margin-top: 0; border-radius: 0; }
  .search-section { padding: 28px 24px 0; }
  .filter-row { padding: 14px 24px 0; }
  .results-header { padding: 24px 24px 12px; }
  .loc-list { padding: 0 24px; }
}
  ...
  <div class="top-header-inner">
    ...
    <nav class="desktop-nav">
      ...
      <a href="{{ route('user.kendaraan') }}">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="3" width="15" height="13" rx="2"/><circle cx="5.5" cy="18.5" r="2.5"/></svg>
        Kendaraan
      </a>
      <a href="{{ route('user.pengaturan') }}">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93l-1.41 1.41M4.93 4.93l1.41 1.41M12 2v2m0 16v2"/></svg>
        Pengaturan
      </a>
    </nav>
    ...
    <div class="header-right">
      <div style="position:relative">
        <div class="map-ctrl-btn" style="background:rgba(255,255,255,0.9);backdrop-filter:blur(12px);cursor:pointer">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
        </div>
        <div style="position:absolute;top:8px;right:8px;width:7px;height:7px;background:var(--red);border-radius:50%;border:1.5px solid #fff"></div>
      </div>
...
<div class="page-wrap">
  ...
  <div class="content-panel">
    ...
    <div class="filter-row">
      @php
        $pills = [
          'all'      => ['icon' => '<line x1="4" y1="6" x2="20" y2="6"/><line x1="8" y1="12" x2="16" y2="12"/><line x1="11" y1="18" x2="13" y2="18"/>',      'label' => 'Semua'],
          'avail'    => ['icon' => '<polyline points="20 6 9 17 4 12"/>',                                                                                        'label' => 'Tersedia'],
          'nearest'  => ['icon' => '<path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/>',                                     'label' => 'Terdekat'],
          'popular'  => ['icon' => '<polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>',       'label' => 'Populer'],
          'cheapest' => ['icon' => '<line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/>',                         'label' => 'Termurah'],
        ];
      ...
      @foreach($pills as $key => $pill)
        <a
          href="{{ route('user.lokasi', array_filter(['filter' => $key, 'q' => $currentQ])) }}"
          class="pill {{ $currentFilter === $key ? 'active' : '' }}"
        >
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">{!! $pill['icon'] !!}</svg>
          {{ $pill['label'] }}
        </a>
      @endforeach
    </div>
      ...
      @forelse($lokasis as $lokasi)
        ...
        <a class="loc-card" href="{{ route('user.lokasi.show', $lokasi->id) }}">
          ...
          <div class="loc-img-wrap">
            @if($lokasi->foto)
              <img src="{{ asset('storage/' . $lokasi->foto) }}" alt="{{ $lokasi->nama }}" loading="lazy">
            @else
            ...
            @if($lokasi->jam_buka && $lokasi->jam_tutup)
              <div class="img-dist">
                ...
                {{ \Carbon\Carbon::parse($lokasi->jam_buka)->format('H:i') }} - {{ \Carbon\Carbon::parse($lokasi->jam_tutup)->format('H:i') }}
              </div>
            @endif
          </div>
            ...
            <div class="loc-footer">
              <div class="loc-price-wrap">
                ...
                <div class="slot-tag {{ $slotClass }}">
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="1" y="3" width="15" height="13" rx="2"/><circle cx="5.5" cy="18.5" r="2.5"/></svg>
                  {{ $slotText }}
                </div>
              </div>
...
const statusBadge = { avail: 'popup-avail', busy: 'popup-busy', full: 'popup-full' };

let mapObj;

function initMap() {
  // Tentukan center dari rata-rata koordinat lokasi, fallback Bogor
  let centerLat = -6.605, centerLng = 106.803;
  if (mapLokasi.length) {
  ...
  mapLokasi.forEach(loc => {
    ...
    L.marker([loc.lat, loc.lng], { icon }).addTo(mapObj).bindPopup(`
      <div>
        ${fotoHtml}
