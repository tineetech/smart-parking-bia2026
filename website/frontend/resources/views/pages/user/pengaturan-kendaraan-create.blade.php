...
<title>Parkify — Tambah Kendaraan</title>
...
.hero-vehicle-name {
  ...
  color: var(--text-primary); letter-spacing: -0.5px; text-align: center;
  transition: color 0.3s; position: relative; z-index: 1;
}
.hero-vehicle-sub { font-size: 12px; color: var(--text-muted); margin-top: 3px; font-weight: 500; text-align: center; position: relative; z-index: 1; }
.hero-plate-chip {
...
.hero-plate-chip.placeholder { color: var(--text-muted); font-weight: 600; font-size: 13px; letter-spacing: 0; }
...
.step-item.done:not(:last-child)::after { background: var(--blue-main); }
.step-num {
...
.step-item.active .step-num { background: var(--blue-main); color: #fff; border-color: var(--blue-main); box-shadow: 0 0 0 3px rgba(37,99,235,0.15); }
...
/* ── ALERT LARAVEL ── */
.alert-error {
  background: var(--red-soft); border: 1.5px solid rgba(239,68,68,0.25);
...
.form-label {
  font-size: 12px; font-weight: 600; color: var(--text-secondary); padding-left: 4px;
  display: flex; align-items: center; gap: 5px;
}
.form-label .required { color: var(--red); font-size: 11px; }
.form-label .optional { color: var(--text-muted); font-size: 10px; font-weight: 500; background: var(--bg-input); padding: 1px 7px; border-radius: 999px; }
...
.form-select {
  background: var(--bg-card); border: 1.5px solid var(--border);
  border-radius: 16px; padding: 14px 44px 14px 18px;
  font-family: 'Poppins', sans-serif; font-size: 14px; font-weight: 500;
  color: var(--text-primary); outline: none; width: 100%;
  transition: border-color 0.18s; box-shadow: var(--shadow-sm);
...
.form-select.is-invalid { border-color: var(--red); box-shadow: 0 0 0 3px rgba(239,68,68,0.08); }
...
.color-dot.selected {
  border-color: var(--blue-main); transform: scale(1.18);
...
.info-box-icon { color: var(--blue-main); flex-shrink: 0; margin-top: 1px; }
.info-box-text { font-size: 12px; color: var(--text-secondary); line-height: 1.6; }
.info-box-text strong { color: var(--blue-main); }

/* ── SUBMIT BUTTON ── */
.submit-btn {
  width: 100%; display: flex; align-items: center; justify-content: center; gap: 10px;
  background: var(--blue-main); color: #fff;
  border: none; border-radius: 18px; padding: 17px 24px;
...
.submit-btn:hover { background: var(--blue-bright); transform: translateY(-2px); box-shadow: 0 6px 20px rgba(37,99,235,0.35); }
.submit-btn:active { transform: scale(0.98); }
.cancel-btn {
  width: 100%; display: flex; align-items: center; justify-content: center; gap: 8px;
  background: transparent; color: var(--text-muted);
  border: none; border-radius: 18px; padding: 13px 24px;
  font-size: 13px; font-weight: 600; font-family: 'Poppins', sans-serif;
  cursor: pointer; transition: color 0.15s; margin-top: 6px; text-decoration: none;
}
...
@media (max-width: 420px) {
  .hero-vehicle-name { font-size: 18px; }
  .jenis-selector { grid-template-columns: 1fr 1fr; }
  .jenis-title { font-size: 12px; }
  .jenis-sub { display: none; }
}
</style>
</head>
<body>
  ...
  <div class="content-inner">
    ...
    <div class="vehicle-hero" id="vehicleHero">
      ...
      <div class="hero-car-wrap" id="heroCarWrap">
        ...
        <svg id="carSvgMotor" style="display:none" width="240" height="140" viewBox="0 0 280 150" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <form action="{{ route('user.kendaraan.store') }}" method="POST" id="formKendaraan" novalidate>
      ...
      <div class="jenis-selector">
        <div class="jenis-option {{ old('jenis','mobil') === 'mobil' ? 'selected' : '' }}" id="jenisOptionMobil" onclick="selectJenis('mobil')">
        ...
        <div class="jenis-option {{ old('jenis') === 'motor' ? 'selected' : '' }}" id="jenisOptionMotor" onclick="selectJenis('motor')">
          <div class="jenis-icon">
      ...
      <div class="form-group">
        ...
        <select class="form-select {{ $errors->has('merek') ? 'is-invalid' : '' }}"
                id="merekKendaraan" name="merek"
                onchange="onMerekChange()">
          ...
          <optgroup label="Mobil" id="groupMobil">
            @foreach(['BMW','Toyota','Honda','Suzuki','Mitsubishi','Daihatsu','Mercedes-Benz','Audi','Hyundai','Kia','Wuling','MG'] as $b)
              <option value="{{ $b }}" {{ old('merek') === $b ? 'selected' : '' }}>{{ $b }}</option>
            @endforeach
          </optgroup>
          {{-- Motor brands --}}
      ...
      <div class="form-group">
        ...
        <input class="form-field {{ $errors->has('model') ? 'is-invalid' : '' }}"
               ...
               placeholder="Contoh: Avanza G, NMAX 155, Civic RS..."
               value="{{ old('model') }}"
      ...
      <div class="checkbox-card {{ old('utama') ? 'checked' : '' }}" id="checkboxCard" onclick="toggleCheckbox()">
...
const modelsByBrand = {
  ...
  'Mitsubishi':   ['Xpander','Xpander Cross','Pajero Sport','Outlander PHEV','Eclipse Cross','L300','Colt L300'],
...
function setStep(n, state) {
  ...
  if (state === 'done') {
    numEl.innerHTML = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>';
  } else {
    numEl.textContent = n;
  }
}
